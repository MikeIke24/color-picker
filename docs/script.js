...
for(let i=0;i<colors.length;i++){
  ...
  for(let j=0;j<colors[i].length;j++){
    colorHtml+=`<div id="co${i}${j}" class="color" data-clipboard-text="${colors[i][j]}" style="background-color:${colors[i][j]};"></div>`;
  }
...
function copiedAnimationColor(id){
  $(`#${id}`).append(`<div id="cco${id}" class="copied-alert-color">Copied!</div>`);
  console.log(id);
  setTimeout(function(){
